/**
 * Adapted megakernel for Qwen3-TTS-12Hz-0.6B talker decoder on RTX 5090.
 *
 * Based on AlpinDale's qwen_megakernel for Qwen3-0.6B. Key changes:
 *   1) Interleaved M-RoPE (mrope_section [24,20,20]) replacing standard RoPE
 *   2) Pre-computed input hidden state (dual-track embed done in Python)
 *   3) Smaller LM head (vocab 3072 vs 151,936) — ~18% speedup
 *   4) Per-step cos/sin buffers (3 position axes merged in Python)
 *
 * The 28-layer transformer backbone is architecturally identical to Qwen3-0.6B.
 */
...
// ============================================================================
// Model constants — Qwen3-TTS talker has same backbone dims as Qwen3-0.6B
// ============================================================================
...
// TTS talker vocab (codebook-0 + special tokens)
#ifndef LDG_TTS_VOCAB_SIZE
#define LDG_TTS_VOCAB_SIZE 3072
#endif
constexpr int LDG_VOCAB_SIZE = LDG_TTS_VOCAB_SIZE;
...
// ============================================================================
// Atomic barrier (identical to original)
// ============================================================================
...
// ============================================================================
// QKV matvec — reads from pre-computed input_hidden (bf16 buffer)
// ============================================================================
...
ldg_matvec_qkv(auto &grid, const __nv_bfloat16 *__restrict__ input,
  ...
  // ALL blocks compute RMSNorm redundantly (eliminates a barrier)
  {
...
// ============================================================================
// Attention with interleaved M-RoPE
// ============================================================================
...
__device__ void ldg_attention_tts(
    ...
    // Pre-computed M-RoPE cos/sin for this step (128 bf16 values each)
    const __nv_bfloat16 *__restrict__ cos_step,
    const __nv_bfloat16 *__restrict__ sin_step,
    // Weights to prefetch
    ...
    unsigned int *__restrict__ kv_flag = nullptr,
    unsigned int *__restrict__ attn_flag = nullptr,
    int layer_idx = 0) {
  ...
  int write_pos = cache_len - 1;

  // -- Fused QK norm with HALF-SPLIT RoPE --
  // Half-split: pair dim d with dim d+64, matching reference rotate_half().
  // Each lane holds kl[0..3] at dims [d, d+32, d+64, d+96] where d=lane_id.
  // Pairs: (kl[0], kl[2]) for (d, d+64) and (kl[1], kl[3]) for (d+32, d+96).
  // cos/sin layout: [θ₀..θ₆₃, θ₀..θ₆₃] (cat(freqs, freqs)).
  //
  // Block 0: K norm + RoPE + KV cache write
  if (block_id == 0) {
    for (int h = warp_id; h < NUM_KV_HEADS; h += LDG_NUM_WARPS) {
      float *k_head = k + h * HEAD_DIM;
      const float *v_head = v + h * HEAD_DIM;
      __nv_bfloat16 *kc =
          k_cache + h * max_seq_len * HEAD_DIM + write_pos * HEAD_DIM;
      __nv_bfloat16 *vc =
          v_cache + h * max_seq_len * HEAD_DIM + write_pos * HEAD_DIM;
...
#pragma unroll
      ...
      // Half-split RoPE: pair (d, d+64) via local registers
      // kl[0]=dim d, kl[1]=dim d+32, kl[2]=dim d+64, kl[3]=dim d+96
      {
        float c0 = __bfloat162float(__ldg(cos_step + lane_id));
        float s0 = __bfloat162float(__ldg(sin_step + lane_id));
        float a0 = kl[0], a2 = kl[2];
        kl[0] = a0 * c0 - a2 * s0;
        kl[2] = a0 * s0 + a2 * c0;

        float c1 = __bfloat162float(__ldg(cos_step + lane_id + 32));
        float s1 = __bfloat162float(__ldg(sin_step + lane_id + 32));
        float a1 = kl[1], a3 = kl[3];
        kl[1] = a1 * c1 - a3 * s1;
        kl[3] = a1 * s1 + a3 * c1;
      }
...
#pragma unroll
      for (int i = lane_id, j = 0; i < HEAD_DIM; i += WARP_SIZE, j++) {
        kc[i] = __float2bfloat16(kl[j]);
        vc[i] = __float2bfloat16(v_head[i]);
      }
    }
  }

  // Attention blocks: Q norm + half-split RoPE
...
#pragma unroll
      ...
      // Half-split RoPE: pair (d, d+64)
      {
        float c0 = __bfloat162float(__ldg(cos_step + lane_id));
        float s0 = __bfloat162float(__ldg(sin_step + lane_id));
        float a0 = ql[0], a2 = ql[2];
        ql[0] = a0 * c0 - a2 * s0;
        ql[2] = a0 * s0 + a2 * c0;

        float c1 = __bfloat162float(__ldg(cos_step + lane_id + 32));
        float s1 = __bfloat162float(__ldg(sin_step + lane_id + 32));
        float a1 = ql[1], a3 = ql[3];
        ql[1] = a1 * c1 - a3 * s1;
        ql[3] = a1 * s1 + a3 * c1;
      }
...
#pragma unroll
  ...
  // Non-attention blocks: prefetch while QK norm runs
  ...
  // Online softmax attention (identical math to original)
  ...
  if (attn_flag) {
    ...
    if (block_id >= ATTN_BLOCKS && threadIdx.x != 0 &&
      ...
      if (prefetch_id % LDG_PREFETCH_BLOCK_STRIDE == 0) {
        ...
        int all_total = o_total + g_total + u_total + d_total;
...
// ============================================================================
// O Projection + Residual + PostNorm + MLP (identical to original)
// ============================================================================
...
#pragma unroll 4
  ...
  // Post-attention RMSNorm (redundant across blocks)
  {
    __shared__ float smem_reduce[LDG_NUM_WARPS];
    float local_sum_sq = 0.0f;
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += LDG_BLOCK_SIZE * 2) {
...
// ============================================================================
// LM head for TTS vocab (3072 rows — fits in a single fused kernel)
// ============================================================================

__global__ void ldg_lm_head_tts_fused(const float *__restrict__ hidden,
...
#pragma unroll
  ...
  // Block 0 does the final reduction across all blocks
  if (blockIdx.x == 0) {
    if (threadIdx.x == 0) {
...
// ============================================================================
// TTS decode kernel — takes pre-computed input_hidden and per-step cos/sin
// ============================================================================

__global__ void __launch_bounds__(LDG_BLOCK_SIZE, 1) ldg_tts_decode_kernel(
    ...
    int num_layers, int cache_len, int max_seq_len, float attn_scale) {

  int block_id = blockIdx.x;
  int num_blocks = gridDim.x;

  // Reset barriers on-device
  if (block_id == 0 && threadIdx.x == 0) {
    *barrier_counter = 0;
    *barrier_sense = 0;
  ...
  for (int layer = 0; layer < num_layers; layer++) {
    ...
    const __nv_bfloat16 *layer_input =
        (layer == 0) ? input_hidden : hidden_buffer;
    ...
    ldg_attention_tts(grid, g_q, g_k, g_v, layer_k_cache, layer_v_cache,
                      g_attn_out, cache_len, max_seq_len, attn_scale,
                      w.q_norm_weight, w.k_norm_weight, cos_step, sin_step,
...
static inline void ldg_configure_kernel_attributes() {
  ...
  if (configured)
    return;
  configured = true;
  cudaFuncSetAttribute(ldg_tts_decode_kernel,
                       cudaFuncAttributePreferredSharedMemoryCarveout,
                       cudaSharedmemCarveoutMaxShared);
  cudaFuncSetAttribute(ldg_lm_head_tts_fused,
                       cudaFuncAttributePreferredSharedMemoryCarveout,
...
extern "C" void launch_ldg_tts_decode(
  ...
  // Reset barriers from the host before each kernel launch to prevent a race
  // condition where non-block-0 blocks see stale barrier_sense values from the
  // previous invocation and pass through the initial sync prematurely.
  ...
  ldg_tts_decode_kernel<<<LDG_NUM_BLOCKS, LDG_BLOCK_SIZE, 0, stream>>>(
      (const __nv_bfloat16 *)input_hidden, layer_weights,
      (const __nv_bfloat16 *)final_norm_weight,
      (const __nv_bfloat16 *)cos_step, (const __nv_bfloat16 *)sin_step,
      ...
      num_layers, cache_len, max_seq_len, attn_scale);

  cudaMemsetAsync(d_lm_head_counter, 0, sizeof(unsigned int), stream);
  ldg_lm_head_tts_fused<<<LDG_LM_NUM_BLOCKS, LDG_LM_BLOCK_SIZE, 0, stream>>>(
